...
namespace System.Buffers.Text
{
    ...
    /// with <see cref="StringComparison.OrdinalIgnoreCase"/>. For example, the accented
    /// characters '&#xE1;' and '&#xC1;' will compare as <em>not equal</em> using the APIs
    /// on this class, even though they may compare as equal using the equality routines
    /// provided by the <see cref="string"/> class.
    ///
    /// All APIs on this class are culture-insensitive.
    /// </remarks>
    public static class Ascii
    {
        ...
        /// <returns><see langword="true"/> if the buffers are equivalent; otherwise, <see langword="false"/>.</returns>
        /// <remarks>
        /// Any non-ASCII bytes within the input buffers are compared for raw binary equivalence.
        /// </remarks>
        public static bool EqualsIgnoreCase(ReadOnlySpan<byte> left, ReadOnlySpan<byte> right)
        {
            ...
            {
                uint leftValue = left[i];
                uint rightValue = right[i];

                // The elements must be an exact match, or they must be equivalent when
                // converted to lowercase. We special-case an exact match first because
                // we expect that to be most common. Moving the XOR operation first
                // allows us to repurpose its result as a fast equality check.

                uint xor = leftValue ^ rightValue;
                if (xor == 0)
                {
                    continue; // values were exactly equal
                }

                if ((xor & ~0x20u) != 0 || !UnicodeUtility.IsInRangeInclusive(leftValue | 0x20u, 'a', 'z'))
                {
                    return false; // values differed beyond 0x20 bit, or one value was not [A-Za-z]
                }
            }
        ...
        /// <returns><see langword="true"/> if the buffers are equivalent; otherwise, <see langword="false"/>.</returns>
        /// <remarks>
        /// Any non-ASCII chars within the input buffers are compared for raw binary equivalence.
        /// </remarks>
        ...
        /// <returns><see langword="true"/> if the buffers are equivalent; otherwise, <see langword="false"/>.</returns>
        /// <remarks>
        /// Any non-ASCII data within the input buffers will cause this method to return <see langword="false"/>.
        /// </remarks>
        ...
        public static bool Equals(ReadOnlySpan<byte> left, string right)
        {
            if (right is null)
            {
                ThrowHelper.ThrowArgumentNullException(ExceptionArgument.right);
            }
        ...
        public static bool EqualsIgnoreCase(ReadOnlySpan<byte> left, ReadOnlySpan<char> right)
        {
            ...
            {
                uint leftValue = left[i];
                if (!UnicodeUtility.IsAsciiCodePoint(leftValue))
                {
                    return false;
                }
        ...
        public static int IndexOfIgnoreCase(ReadOnlySpan<byte> text, ReadOnlySpan<byte> value)
        {
            if (value.IsEmpty)
            {
                return 0; // empty substring trivially occurs at beginning of search space
            }

            uint valueFirstByte = value[0];
            uint orMask = 0;

            if (UnicodeUtility.IsInRangeInclusive(valueFirstByte | 0x20u, 'a', 'z'))
            {
                // First byte of needle is an ASCII char [A-Za-z]. As an optimization,
                // we'll convert the search space to lowercase as we look for this char.

                orMask = 0x20u;
                valueFirstByte |= orMask;
            }
            ...
            ReadOnlySpan<byte> valueRemainder = value.Slice(1);

            for (int i = 0; i <= lastIdx; i++)
            {
                if ((text[i] | orMask) == valueFirstByte && EqualsIgnoreCase(text.Slice(i + 1, valueRemainder.Length), valueRemainder))
                {
                    return i;
                }
            }
        ...
        public static int IndexOf(ReadOnlySpan<byte> text, ReadOnlySpan<char> value)
        {
            ...
            if (!UnicodeUtility.IsAsciiCodePoint(valueFirstChar))
            {
                return -1; // can never locate a non-ASCII needle
            }
            ...
            int lastIdx = text.Length - value.Length;
            ReadOnlySpan<char> valueRemainder = value.Slice(1);

            for (int i = 0; i <= lastIdx; i++)
            {
                if (text[i] == valueFirstChar && Equals(text.Slice(i + 1, valueRemainder.Length), valueRemainder))
                {
                    return i;
                }
            }
        ...
        public static int IndexOfIgnoreCase(ReadOnlySpan<byte> text, ReadOnlySpan<char> value)
        {
            ...
            if (UnicodeUtility.IsInRangeInclusive(valueFirstChar | 0x20u, 'a', 'z'))
            {
                // First char of needle is an ASCII char [A-Za-z]. As an optimization,
                // we'll convert the search space to lowercase as we look for this char.

                orMask = 0x20u;
                valueFirstChar |= orMask;
            }
            ...
            for (int i = 0; i <= lastIdx; i++)
            {
                if ((text[i] | orMask) == valueFirstChar && EqualsIgnoreCase(text.Slice(i + 1, valueRemainder.Length), valueRemainder))
                {
                    return i;
                }
            }
        ...
        public static int ToLower(ReadOnlySpan<byte> source, Span<byte> destination)
        {
            ...
            ToLower(ref MemoryMarshal.GetReference(source), ref MemoryMarshal.GetReference(destination), (uint)source.Length);
        ...
        private static void ToLower(ref byte source, ref byte destination, nuint count)
        {
            nuint offset = 0;

            if (count >= sizeof(uint))
            {
                nuint lastOffsetWhereCanIterate = count - sizeof(uint);

                do
                {
                    uint srcRawBytes = Unsafe.ReadUnaligned<uint>(ref Unsafe.Add(ref source, (nint)offset));
                    if (!ASCIIUtility.AllBytesInUInt32AreAscii(srcRawBytes))
                    {
                        break; // our worker loop is only optimized for all-ASCII data
                    }

                    uint dstRawBytes = ASCIIUtility.ConvertAllAsciiBytesInUInt32ToLowercase(srcRawBytes);
                    Unsafe.WriteUnaligned<uint>(ref Unsafe.Add(ref destination, (nint)offset), dstRawBytes);
                } while ((offset += sizeof(uint)) <= lastOffsetWhereCanIterate);
            }

            // We saw non-ASCII data or we're running out of space. Fall down slow path.

            for (; offset < count; offset++)
            {
                Unsafe.Add(ref destination, (nint)offset) = ToLower(Unsafe.Add(ref source, (nint)offset));
            }
        }
