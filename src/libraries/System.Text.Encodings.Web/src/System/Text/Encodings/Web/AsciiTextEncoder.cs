...
#error throw exception here
        ...
        {
            int idx = (int)AsciiEncoder.FindIndexOfFirstByteToEncode(
                ref MemoryMarshal.GetReference(utf8Text),
                (uint)utf8Text.Length,
                in _state);

            Debug.Assert(idx <= utf8Text.Length);
            return (idx < utf8Text.Length) ? idx : -1; // -1 signifies no data requires encoding
        }
